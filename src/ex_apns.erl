...
connect(Address, Port, {CertFile, Pass}) -> connect(Address, Port, CertFile, Pass);
connect(Address, Port, CertFile) -> connect(Address, Port, CertFile, undefined).

connect(Address, Port, CertFile, Pass) ->
  CaCertFile = filename:join([code:priv_dir(?MODULE), "entrust_2048_ca.cer"]),
  SslOptions = [binary,
                {active, false},
                {certfile, CertFile},
                {cacertfile, CaCertFile}],
  SslOptions2 = if (Pass /= undefined) -> [{password, Pass} | SslOptions]; true -> SslOptions end,
  ssl:connect(Address, Port, SslOptions2).
        ...
        Err ->
            case Err of
                undefined ->
                    ...
                    ?ERROR_MSG(Format, [?MODULE, name()]);
                {Identifier, Status} ->
                    Format = "~w[~w]: could not send extended notification ~B (~w)~n",
                    ?ERROR_MSG(Format, [?MODULE, name(), Identifier, Status])
            end,
...
feedback_loop(Socket, Acc) ->
  case ssl:recv(Socket, 39, infinity) of
    {ok, <<8, Timestamp:32, 32:16, TokenInt:256>>} ->
      TokenU = token_to_binary(TokenInt),
